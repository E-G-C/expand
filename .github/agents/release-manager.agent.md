...
- Load `tag-driven-release-versioning` for tag-based version sync or manifest bump questions.
- Load `release-pipeline-parity` when reconciling GitHub Actions and Azure DevOps behavior.
- Load `release-writeback-via-pr` when the default branch is protected or a direct workflow push is blocked.
