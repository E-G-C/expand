...
- Do not claim the issue is fixed without running fresh verification; use `verification-before-completion`.
- If multiple failures are truly independent, use `parallel-dispatch` only after confirming they do not share a root cause.
